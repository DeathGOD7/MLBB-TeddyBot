...
#### VERSION ####
version = "BETA Release Candidate Ver.01.50 (20210713)"
...
async def _overall(ctx, region="All", mode="All", elo="All", period="Week", sort="Top", role="null", view="normal", about="null"):
    ...
    #await ctx.send(f"Running Stats... Region:  {region}, Mode: {mode}, Elo: {elo}, Date Range: {period}")
    if about!="null":
        if about=="show":
            about_title = "Teddy"
...
async def test(ctx, hero: str, region="All", mode="All", elo="All", period="Week", show="null"):
    await ctx.send(f":bear: `Processing request...`")
    #await ctx.send(content=f"Searching TierData for...  '{hero}'")
